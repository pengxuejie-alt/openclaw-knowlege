...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() =>
    enqueueGlobal(async () => {
      ...
      // When the user has uploaded images or the prompt references images,
      // but the primary model doesn't support images, we should use the configured
      // imageModel instead (if available).
      const hasUploadedImages = (params.images?.length ?? 0) > 0;
      if (hasUploadedImages || params.prompt) {
        // First resolve the primary model to check if it supports images
        const { model: primaryModel } = resolveModel(provider, modelId, agentDir, params.config);
        const primarySupportsImages = primaryModel?.input?.includes("image") ?? false;

        // Only switch if primary doesn't support images and we have images
        if (!primarySupportsImages && hasUploadedImages && params.config) {
          // Import model-input helper
          const { resolveAgentModelPrimaryValue } = await import("../../config/model-input.js");
          const imageModelPrimary = resolveAgentModelPrimaryValue(
            params.config.agents?.defaults?.imageModel,
          );

          if (imageModelPrimary?.trim()) {
            const slashIdx = imageModelPrimary.indexOf("/");
            if (slashIdx > 0 && slashIdx < imageModelPrimary.length - 1) {
              const imageProvider = imageModelPrimary.slice(0, slashIdx).trim();
              const imageModelId = imageModelPrimary.slice(slashIdx + 1).trim();

              // Verify the imageModel actually supports images
              const { model: imageModelDef } = resolveModel(
                imageProvider,
                imageModelId,
                agentDir,
                params.config,
              ...
              if (imageModelDef && imageModelDef.input?.includes("image")) {
                // Switch to imageModel for this request
                provider = imageProvider;
                modelId = imageModelId;
                log.info(
                  `Auto-switched to imageModel ${imageModelPrimary} ` +
                    `(primary model ${params.provider}/${params.model} does not support images)`,
                );
              } else {
                log.warn(
                  `Configured imageModel ${imageModelPrimary} does not support images, ` +
                    `falling back to primary model ${params.provider}/${params.model}`,
                );
              }
            }
          } else {
            log.warn(
              `User uploaded images but primary model ${params.provider}/${params.model} ` +
                `does not support images and no imageModel is configured. Images will be discarded.`,
            );
          }
        }
      }
